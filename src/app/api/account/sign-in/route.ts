...
export async function POST(request: Request) {
  try {
    ...
    const account = await authenticateLocalAccount(body.username ?? "", body.pin ?? "");
    ...
    return Response.json({ ok: false, error: error instanceof Error ? error.message : "Sign-in failed." }, { status: 401 });
  }
}
