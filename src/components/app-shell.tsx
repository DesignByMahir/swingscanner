        ...
        <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
          ...
          <DialogContent className="theme-dialog max-h-[90dvh] overflow-y-auto sm:max-w-[780px] gap-0 p-0">
            ...
            <div className="border-b p-5">
              ...
              {!account.configured ? (
                <p className="mt-2 text-xs text-muted-foreground">Account sync is unavailable in this build. Local journal data still works normally.</p>
              ) : account.userId ? (
                <div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-muted-foreground">Journal entries, reflections, and flagged setups follow this account across devices.</p><Button variant="outline" size="sm" onClick={() => account.signOut()}>Sign out</Button></div>
              ) : (
                <>
                  <p className="mt-2 text-xs text-muted-foreground">Use a username and exactly six numbers. Create the account once, then use the same login on Windows or Mac.</p>
